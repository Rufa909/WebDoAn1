document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadBookings() {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/booking/business/${user.id}`);
            const data = await response.json();
    ...
    function renderBookings() {
        ...
        if (currentFilter !== 'all') {
            filteredBookings = filteredBookings.filter(b => b.trangThai === currentFilter);
        }
        ...
        if (filteredBookings.length === 0) {
            containerEl.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Chưa có yêu cầu đặt phòng ${currentFilter !== 'all' ? 'trong danh mục này' : ''}</p>
                </div>
            `;
            return;
        }

        containerEl.innerHTML = filteredBookings.map(booking => createBookingCard(booking)).join('');
    ...
    function createBookingCard(booking) {
        ...
        const status = statusMap[booking.trangThai] || { text: booking.trangThai, class: '' };
    ...
    async function confirmBooking(bookingId) {
        try {
            const response = await fetch(`${API_BASE_URL}/api/booking/confirm/${bookingId}`, {
                method: 'PUT',
                credentials: 'include'
            });
    ...
    async function rejectBooking(bookingId, reason) {
        try {
            const response = await fetch(`${API_BASE_URL}/api/booking/reject/${bookingId}`, {
                method: 'PUT',
    ...
    function updateNotificationBadge() {
        const pendingCount = allBookings.filter(b => b.trangThai === 'choXacNhan').length;
    ...
    function openPanel() {
        const pendingBookings = allBookings.filter(b => b.trangThai === 'choXacNhan');
        const panelBody = document.getElementById('notification-panel-body');
        
        if (pendingBookings.length === 0) {
            panelBody.innerHTML = '<p style="text-align: center; color: #999; padding: 20px;">Không có thông báo mới</p>';
        } else {
            panelBody.innerHTML = pendingBookings.map(b => `
                <div class="notification-item" style="padding: 15px; border-bottom: 1px solid #f0f0f0;">
                    <div style="display: flex; gap: 10px;">
                        ...
                        <div style="flex: 1;">
                            <p style="margin: 0 0 5px 0;"><strong>${b.hoTen}</strong> đặt phòng <strong>${b.tenPhong}</strong></p>
